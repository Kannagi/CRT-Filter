...
void VidSoftFXApplyEffect(unsigned char* ps, unsigned char* pd, int nPitch)
{
	...
	switch (nSoftFXBlitter) {
		...
		case FILTER_CRTx22: {
			CRTx22(ps,pd,nSoftFXImageWidth,nSoftFXImageHeight, nPitch, nPitch);
			break;
		}
		case FILTER_CRTx33: {
			CRTx33(ps,pd,nSoftFXImageWidth,nSoftFXImageHeight, nPitch, nPitch);
			break;
		}
		case FILTER_CRTx44: {
			CRTx44(ps,pd,nSoftFXImageWidth,nSoftFXImageHeight, nPitch, nPitch);
			break;
		}
	}
}
